...
def translit_file(oovfile, smt):
    ...
    with open('{}/translit.out'.format(data.run), 'w', encoding='utf-8') as out:
        Popen('moses -f {}/data/train/lowercased/model/hpb/transliterate/tuning/moses.tuning.ini -i {}/translit.in'.format(os.environ['THESISDIR'], data.run).split(), stdout=out, universal_newlines=True).wait()
        out.flush()
        with open('{}/en.out'.format(data.run)) as enout:
            for k in d.keys():
